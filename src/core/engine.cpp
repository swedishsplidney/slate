...
Ray Engine::screenPointToRay(glm::vec2 mousePos) {
  glm::vec2 vpOffset = m_viewportPanel ? m_viewportPanel->getAbsolutePosition()
                                       : glm::vec2(0.0f);
  glm::vec2 vpSize = m_viewportPanel ? m_viewportPanel->getSize()
                                     : glm::vec2(static_cast<float>(m_width),
                                                 static_cast<float>(m_height));

  // viewport scale correction
...
bool Engine::isPointInElement(glm::vec2 point,
...
Engine::Engine() {
  ...
  m_uiManager->loadFont("inter", "assets/fonts/Inter-4.1/InterVariable.ttf",
                        16.0f);
  m_uiManager->loadFont(
      "jetbrains",
      "assets/fonts/JetBrains_Mono/JetBrainsMono-VariableFont_wght.ttf", 15.0f);
  ...
  if (fontLoader) {
    ...
    vkRenderer->createFontTexture(fontLoader->getAtlasBitmap().data(),
  ...
  MenuHeader editMenu{
      "Edit",
      {{"Undo", "editor.undo", "Ctrl+Z"}, {"Redo", "editor.redo", "Ctrl+Y"}}};
...
void Engine::registerDefaultCommands() {
  m_commandRegistry->registerCommand(
      "file.import_mesh", [](const CommandRegistry::CommandArgs &args) {
        std::string path = args.empty() ? "" : args[0];
        return std::make_unique<ImportMeshCommand>(path);
      });

  m_commandRegistry->registerCommand(
      "view.toggle_grid", [](const CommandRegistry::CommandArgs &) {
        return std::make_unique<ToggleGridCommand>();
      });

  m_commandRegistry->registerCommand(
      "editor.set_gizmo_translate", [](const CommandRegistry::CommandArgs &) {
        return std::make_unique<SetGizmoModeCommand>(
            SetGizmoModeCommand::Mode::Translate);
      });

  m_commandRegistry->registerCommand(
      "editor.set_gizmo_rotate", [](const CommandRegistry::CommandArgs &) {
        return std::make_unique<SetGizmoModeCommand>(
            SetGizmoModeCommand::Mode::Rotate);
      });

  m_commandRegistry->registerCommand(
      "editor.undo", [this](const CommandRegistry::CommandArgs &) {
        m_commandRegistry->getHistory().undo(m_commandContext);
        return nullptr;
      });

  m_commandRegistry->registerCommand(
      "editor.redo", [this](const CommandRegistry::CommandArgs &) {
        m_commandRegistry->getHistory().redo(m_commandContext);
        return nullptr;
      });

  m_commandRegistry->registerCommand(
      "editor.translate_mesh",
      [this](const CommandRegistry::CommandArgs &args) {
        glm::vec3 delta(0.0f, 0.5f, 0.0f);
        return std::make_unique<TranslateMeshCommand>(m_selectedMeshIndex,
                                                      delta);
      });

  m_commandRegistry->registerCommand(
      "editor.rotate_mesh", [this](const CommandRegistry::CommandArgs &args) {
        glm::quat deltaRot =
            glm::angleAxis(glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        return std::make_unique<RotateMeshCommand>(m_selectedMeshIndex,
                                                   deltaRot);
      });
  ...
  m_commandRegistry->registerCommand(
      "material.update_float",
      [this](const CommandRegistry::CommandArgs &args)
          -> std::unique_ptr<ICommand> {
        if (args.size() < 3)
          return nullptr;
        uint32_t matId = static_cast<uint32_t>(std::stoul(args[0]));
        MaterialPropertyType prop =
            static_cast<MaterialPropertyType>(std::stoi(args[1]));
  ...
  m_commandRegistry->registerCommand(
      ...
      [this](const CommandRegistry::CommandArgs &args)
        ...
        MaterialPropertyType prop =
            static_cast<MaterialPropertyType>(std::stoi(args[1]));
        glm::vec4 val(std::stof(args[2]), std::stof(args[3]),
...
int Engine::checkGizmoHit(const glm::vec2 &mousePos) {
  auto &sceneMeshes =
      static_cast<VulkanRenderer *>(m_renderer.get())->getSceneMeshes();
  if (sceneMeshes.empty() || m_selectedMeshIndex >= sceneMeshes.size() ||
      !sceneMeshes[m_selectedMeshIndex]) {
    return -1;
  }
  ...
  glm::vec3 gizmoCenter = glm::vec3(
      mesh->getModelMatrix() * glm::vec4(mesh->getGeometricCenter(), 1.0f));
  ...
  auto getValidScreenPos = [&](const glm::vec3 &worldPos,
                               glm::vec2 &outScreen) {
    glm::vec4 clipPos = proj * view * glm::vec4(worldPos, 1.0f);
    if (clipPos.w <= 0.001f)
      return false;
  ...
  glm::mat3 assetCorrection = glm::mat3(glm::rotate(
  ...
  if (gizmoMode == VulkanRenderer::GizmoMode::Translate) {
    ...
    auto checkCircleHit = [&](const glm::vec3 &uAxis, const glm::vec3 &vAxis) {
      ...
      for (int i = 0; i <= segments; ++i) {
        float angle =
            (static_cast<float>(i) / segments) * 2.0f * glm::pi<float>();
        glm::vec3 worldPos =
            gizmoCenter +
            gizmoScale * (std::cos(angle) * uAxis + std::sin(angle) * vAxis);
        glm::vec2 screenPos;
  ...
  while (!shouldClose) {
    ...
    while (SDL_PollEvent(&event)) {
      ...
      if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN) {
        ...
        if (clickInsideViewport && (event.button.button == SDL_BUTTON_LEFT ||
        ...
        if (event.button.button == SDL_BUTTON_RIGHT && clickInsideViewport) {
          ...
          if (hitAxis != -1) {
            // gizmo
            auto &sceneMeshes = static_cast<VulkanRenderer *>(m_renderer.get())
                                    ->getSceneMeshes();
            if (!sceneMeshes.empty() &&
              ...
              glm::vec3 gizmoCenter =
                  glm::vec3(mesh->getModelMatrix() *
                            glm::vec4(mesh->getGeometricCenter(), 1.0f));
              ...
              if (intersectRayPlane(ray.origin, ray.direction,
                                    m_gizmoPlaneOrigin, m_gizmoPlaneNormal,
                                    m_lastRayIntersection)) {
            ...
            if (m_inspectorPanel) {
              ...
              if (m_selectedMeshIndex >= 0 &&
                  m_selectedMeshIndex < sceneMeshes.size() &&
                  sceneMeshes[m_selectedMeshIndex]) {
                m_inspectorPanel->setTargetObject(
                    sceneMeshes[m_selectedMeshIndex]->getName());

                if (!m_isDraggingGizmo) {
                  glm::vec3 currentPos = glm::vec3(
                      sceneMeshes[m_selectedMeshIndex]->getModelMatrix()[3]);
                  m_inspectorPanel->setPositionValues(currentPos);
                }
              } else {
                m_inspectorPanel->setTargetObject("None");
      ...
      if (event.type == SDL_EVENT_MOUSE_MOTION) {
        if (m_isDraggingGizmo) {
          ...
          if (!sceneMeshes.empty() &&
            ...
            if (vkRenderer->getGizmoMode() ==
                VulkanRenderer::GizmoMode::Translate) {
              Ray ray = screenPointToRay(currentMousePos);
              glm::vec3 currentIntersection;
              if (intersectRayPlane(ray.origin, ray.direction,
                                    m_gizmoPlaneOrigin, m_gizmoPlaneNormal,
                                    currentIntersection)) {
                glm::vec3 frameWorldDelta =
                    currentIntersection - m_gizmoLastIntersection;
                m_gizmoLastIntersection = currentIntersection;

                float frameAxisDelta =
                    glm::dot(frameWorldDelta, m_dragWorldAxis);
                glm::vec3 translationDelta = m_dragWorldAxis * frameAxisDelta;

                if (glm::length(translationDelta) > 0.0001f) {
                  TranslateMeshCommand translateCmd(m_selectedMeshIndex,
                                                    translationDelta);
            ...
            } else if (vkRenderer->getGizmoMode() ==
                       VulkanRenderer::GizmoMode::Rotate) {
              float sensitivity = 0.01f;
              float deltaAngle =
                  (event.motion.xrel - event.motion.yrel) * sensitivity;

              if (std::abs(deltaAngle) > 0.0001f) {
                glm::quat deltaRot =
                    glm::angleAxis(deltaAngle, m_dragWorldAxis);
      ...
      if (event.type == SDL_EVENT_WINDOW_RESIZED ||
        ...
        if (m_uiManager) {
          m_uiManager->setScreenSize(static_cast<float>(w),
                                     static_cast<float>(h));
    ...
    if (m_uiManager && m_uiManager->isDirty()) {
      ...
      static_cast<VulkanRenderer *>(m_renderer.get())
          ->updateUIGeometryBuffers(vertices, indices);
    }
